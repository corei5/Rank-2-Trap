...
def create_model(cfg):
    ...
    elif cfg.dataset == 'papers':
        # Node features = sentence embeddings (continuous) -> Linear encoder.
        # nfeat_node MUST match your embedder's dim (all-MiniLM-L6-v2 = 384).
        # Edge attr = single integer edge-type id -> Discrete encoder, nfeat_edge=1.
        nfeat_node = 384
        nfeat_edge = 1
        node_type = 'Linear'
        edge_type = 'Linear'
        nout = cfg.model.papers_nout

    if cfg.metis.n_patches > 0:
        if cfg.jepa.enable:
            # ─────────────────────────────────────────────────────────────
            # EMA momentum for the anti-collapse target network. Read from
            # cfg.jepa.ema_momentum if present, else default to 0.996.
            # getattr avoids a crash for datasets whose yaml lacks the key.
            # ─────────────────────────────────────────────────────────────
            ema_momentum = getattr(cfg.jepa, 'ema_momentum', 0.996)
            return GraphJepa(
